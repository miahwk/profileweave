...
function useSelfCheckStartURL() {
  model.value.startURL = `${globalThis.location.origin}/self-check`
}
</script>
        ...
        <div class="editor__body">
          ...
          <section aria-labelledby="basic-title">
            ...
            <div class="field-grid">
              ...
              <div class="field"><label for="profile-start-url">启动页 <b>*</b></label><input id="profile-start-url" v-model.trim="model.startURL" required type="url" placeholder="https://example.com" /><button class="inline-action" type="button" @click="useSelfCheckStartURL">使用本地自检页</button></div>
...
.field-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; }.field-grid--proxy { margin-top: 15px; }.field--wide { grid-column: 1 / -1; }.field { display: grid; align-content: start; gap: 7px; color: var(--muted); font-size: 10px; font-weight: 700; }.field > span b,.field > label b { color: var(--danger); }.field small { color: var(--text-soft); font-size: 9px; font-weight: 450; line-height: 1.45; }.field .field-error { color: #ffaaa5; }
...
.inline-action { width: fit-content; padding: 0; border: 0; color: var(--blue); background: transparent; font-size: 9px; font-weight: 650; cursor: pointer; }

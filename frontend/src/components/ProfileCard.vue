...
const statusLabel = () => {
  if (props.session?.status === 'starting') return '启动中'
  if (props.session?.status === 'running') return '运行中'
  if (props.session?.status === 'stopping') return '停止中'
  if (failed()) return '启动失败'
  return '已停止'
}
  ...
  <article class="profile-card" :class="{ 'profile-card--running': running() }">
    <header>
      ...
      <div class="profile-title">
        <div class="title-row">
          <h3>{{ profile.name }}</h3>
          <span class="run-status" :class="{ 'run-status--active': running(), 'run-status--failed': failed() }">
    ...
    <p v-else-if="failed()" class="failed-note">{{ session?.lastError || '浏览器进程启动失败，请检查本机浏览器与配置。' }}</p>
...
.run-status { display: inline-flex; align-items: center; gap: 5px; flex: 0 0 auto; color: var(--text-soft); font-size: 9px; font-weight: 700; }.run-status i { width: 5px; height: 5px; border-radius: 50%; background: currentColor; }.run-status--active { color: var(--mint); }.run-status--failed { color: #ffaaa5; }
...
.session-note,.blocked-note,.failed-note { margin: 9px 0 0; color: var(--text-soft); font-size: 9px; }.blocked-note,.failed-note { color: #ffaaa5; }.failed-note { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

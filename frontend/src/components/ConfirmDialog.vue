...
const props = withDefaults(defineProps<{
  ...
  confirmLabel?: string
  busyLabel?: string
}>(), { error: '', confirmLabel: '确认删除', busyLabel: '删除中…' })
...
<template>
  <Teleport to="body">
    <div v-if="open" class="dialog-layer" role="presentation" @keydown.esc="cancel">
      <button class="dialog-backdrop" aria-label="关闭确认对话框" :disabled="busy" @click="cancel"></button>
      <section class="confirm-dialog" role="alertdialog" aria-modal="true" aria-labelledby="confirm-title" aria-describedby="confirm-description">
        ...
        <div class="dialog-actions">
          ...
          <button class="button button--danger" type="button" :disabled="busy" autofocus @click="$emit('confirm')">{{ busy ? busyLabel : confirmLabel }}</button>
        </div>
      </section>
    </div>
  </Teleport>
...
.dialog-error { margin-top: -10px; color: #ffc1bd; }

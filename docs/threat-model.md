...
- 只有本机用户主动打开的管理台能够创建、修改、启动和停止 Profile；同一用户的本地
  `--shutdown` 控制命令只能请求统一退出，不能修改 Profile。
...
flowchart LR
  ...
  PROC --> WEB["互联网 / 代理"]
  BIN["服务端发现的浏览器二进制"] --> PROC
```

管理台与 Go 服务处于同一应用信任域；浏览器页面、互联网、代理服务器、系统浏览器二进制和任何第三方 runtime 均不受信任。操作系统账户与本机文件权限属于外部可信基础，ProfileWeave 不是磁盘加密或多用户权限系统。
...
| 浏览器参数/命令注入 | 不接受 Profile/HTTP 提供的可执行文件路径；只从服务端固定候选位置发现浏览器；`exec.Command(executable, args...)` 不经过 shell | 已安装浏览器在发现后被同一 OS 用户替换仍可能执行不同代码 |
| 数据删除后残留 | 删除时拒绝运行中 Profile，并把 browser-data 原子移动到应用回收区；仓储失败时回滚；UI 提供恢复和二次确认的永久删除 | 删除到永久清理之间，回收区仍包含当前 OS 用户可访问的敏感数据；尚无自动保留期限 |
| Profile 并发运行 | 应用服务单实例检查，数据目录按 ID 独占 | 服务崩溃后的未知子进程暂不自动接管 |
| 代理泄漏 | 仅支持无认证代理，不落密码；能力报告明确代理不是 VPN | DNS、扩展、浏览器自身服务和 WebRTC 行为受版本影响 |
| 第三方运行时供应链 | 只发现系统浏览器；不自动下载；要求显示来源/许可/版本；任意自定义路径在初始版本中禁用 | 系统浏览器的真实性仍由用户/OS 负责 |
| Release 被篡改 | Release 输出 SHA256、SBOM、provenance；版本元数据注入 | Windows Authenticode 与 macOS notarization 尚未完成 |
| 本地端口被其他程序占用 | 重复启动与退出命令先验证 `/health` 产品标识；不按未验证 PID 打开或终止 | 同一用户下的恶意进程仍可模拟本地协议，不能替代 OS 身份隔离 |
| 卸载误删 Profile | 安装目录与 `%APPDATA%` 数据目录分离；卸载清单不包含用户数据 | 用户手动删除数据目录仍不可恢复，需先备份 |

## 4. 明确不覆盖

- 已获得当前 OS 用户权限的恶意程序、键盘记录、内存读取或磁盘窃取。
- 网站通过网络、TLS、行为、账号关系或浏览器漏洞识别用户。
- 第三方代理、扩展、系统浏览器或内核的隐私与安全保证。
